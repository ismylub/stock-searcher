...
def get_all_korean_tickers_for_search():
    ...
    try:
        ...
        for _, r in krx.iterrows():
            ...
            full_map[str(r["Name"])] = code + suffix
    except:
        pass
    return full_map
...
# 🔑 API 및 기업 개요 조회
# =======================================================================
MULTIPLE_GEMINI_KEYS = [
    "AIzaSyDitz98ZneZsNP6PCxKrGH62RMC4Dq3CaY", "AIzaSyAnPqbj2OmFVEqPGuhNYHfDPlXo-Uf5FlI",
    "AIzaSyDqAVApvnhonDDxZYNOXuGvPVULU5qo_VU", "AIzaSyDtkX0dDH1Ais63Yv2VIYmcAu_V6B7NuEE",
    "AIzaSyAEc0hWHbfeOS5zhQHrdI59yiy33a_6EhU", "AIzaSyBiElScd2va3xK006klQNmsgVOHGLiGRoE",
    "AIzaSyCwrpRqf_i5jUQlSApxY52xwkvLFFPwle0", "AIzaSyA7rHhuSsKBEU__-a7MFcgZlquwoNedSKg",
    "AIzaSyDEzwuP1cDEFlw03M0zuP3v35zuDJha21Q", "AIzaSyAlj9ujzStC2L6ipzTdvEXm2wRl7hEEmKA",
]

def load_keys_safely():
    return [k.strip() for k in MULTIPLE_GEMINI_KEYS if k.strip().startswith("AIzaSy")]

def fetch_company_profile_only(ticker, market_type):
    if "한국" not in market_type:
        return "미국 우량 기업 (사전 지식 100% 활용하여 작성할 것)"
    ...
    try:
        ...
        html = get_urllib_opener().open(req, timeout=5).read().decode("euc-kr", "ignore")
        profile_match = re.search(
            r'<div class="cmp_comment">(.*?)</div>', html, re.DOTALL
        )
        if profile_match:
            return (
                re.sub("<[^<]+?>", "", profile_match.group(1).strip())
                .replace("\n", " ")
                .replace("\t", " ")
            )
    except:
...
def ask_gemini_analyst(
    ...
    latest_date_str = str(df.index[-1]).split()[0]
    current_price = float(df["Close_line"].iloc[-1])
    try:
        kwargs = {}
        if USE_PROXY: kwargs['proxy'] = PROXY_IP
        ex_df = yf.download("KRW=X", period="5d", progress=False, **kwargs)
        current_exchange_rate = float(ex_df["Close"].iloc[-1].squeeze())
        exchange_rate_info = f"현재 실제 원/달러 환율: {current_exchange_rate:,.1f}원"
    except:
    ...
    broker_inst = (
        ...
        else "■ 3.5. 글로벌 유명 증권사 컨센서스\n- 구글 검색 도구로 글로벌 투자은행 리포트 검색."
    )

    prompt = f"""
    당신은 수석 전략가이자, 4명의 최고 투자 전문가(모멘텀, 가치, 균형, 역추세)로 구성된 '주식 투자 위원회'의 간사입니다.
    검색 기준일: {latest_date_str}
    종목명: {comp_name}({ticker}) | 현재가: {current_price:,.2f}
    {exchange_rate_info}
    {fundamental_info}

    [위원회 사전 평가 점수]
    - 모멘텀 전문가(Gemini): {ai_scores.get("gemini", 0)}점
    - 균형성장 전문가(ChatGPT): {ai_scores.get("gpt", 0)}점
    - 가치투자 전문가(Claude): {ai_scores.get("claude", 0)}점
    ...
    for idx, api_key in enumerate(valid_keys):
        try:
            ...
            if response.status_code == 200:
                ...
                return f"🟢 **{idx + 1}번 API 키 발간 성공**\n\n{answer}"
        except:
            continue
    return "🚨 API 키 소진 또는 에러 발생!"
...
def map_krx_sector_to_gics(sec):
    if not isinstance(sec, str) or sec == "nan" or not sec.strip():
        return "미분류"
    s = sec.lower()
    if any(
    ...
    if any(
        x in s
        for x in ["의약", "의료", "보건", "바이오", "자연과학 및 공학 연구개발업"]
    ):
        return "헬스케어"
    if any(
        x in s
        for x in ["금융", "은행", "보험", "증권", "신탁", "투자", "지주회사", "여신"]
    ):
        return "금융"
    if any(
        x in s
        ...
        return "임의소비재"
    if any(x in s for x in ["방송", "출판", "영상", "통신업", "정보서비스", "광고"]):
        return "커뮤니케이션"
    if any(
        x in s
        for x in [
        ...
        return "소재"
    if any(x in s for x in ["식료품", "음료", "담배", "농업", "수산", "축산", "작물"]):
        return "필수소비재"
    if any(x in s for x in ["전기", "가스", "증기", "수도", "폐기물", "환경"]):
        return "유틸리티"
    if any(x in s for x in ["석유", "석탄", "광업"]):
        return "에너지"
    if any(x in s for x in ["부동산", "임대"]):
        return "부동산"
    if any(
        x in s
        for x in [
...
def sync_market_sectors_v8(market_type):
    ...
    if "한국" in market_type:
        try:
            ...
            sym_col = "Symbol" if "Symbol" in krx.columns else "Code"
            sec_col = "Sector" if "Sector" in krx.columns else "Industry"

            if sym_col and sec_col:
                krx[sym_col] = krx[sym_col].astype(str).str.zfill(6)
                krx_map = dict(zip(krx[sym_col], krx[sec_col]))

                for tk in tickers:
                    if tk not in db or db[tk] == "미분류":
                        pure_code = tk.split(".")[0]
                        official_sec = str(krx_map.get(pure_code, ""))
                        mapped_sec = map_krx_sector_to_gics(official_sec)
                        if mapped_sec != "미분류":
                            db[tk] = mapped_sec
                            changed = True
        except:
...
def start_100b_dashboard():
    ...
    st.markdown(
        """
        <style>
            [data-testid="stSidebarUserContent"] { padding-top: 0rem !important; margin-top: -65px !important; }
            ...
            if not filtered_list:
                ...
                if sel_tk != "NONE":
                    ...
                    if df.empty:
                        ...
                        if st.button(
                            "🤖 4인 전문가 위원회 전략 리포트 발간",
                            use_container_width=True,
                            type="primary",
                        ):
                            with st.spinner(
                                "헤지펀드 수석 분석가 위원회가 데이터를 회의 중입니다..."
                            ):
                                prof = fetch_company_profile_only(sel_tk, c_m)
                                st.session_state[f"ai_{sel_tk}"] = ask_gemini_analyst(
                                    sel_tk,
                                    n_map.get(sel_tk, "종목"),
                                    df,
                                    c_m,
                        ...
                        if f"ai_{sel_tk}" in st.session_state:
                            st.markdown(
                                f"""
                                <style>
                                    div[data-testid="element-container"]:has(.my-sticky-header), div.stElementContainer:has(.my-sticky-header) {{
                                        position: sticky; top: 2.8rem; z-index: 999;
                                        background-color: var(--background-color, white); padding: 5px 0px;
                                    }}
                                </style>
                                <div class="my-sticky-header" style="padding: 10px 0px 10px 0px; border-bottom: 3px solid #ff4b4b; margin-bottom: 20px;">
                                    <h3 style="margin: 0; color: #ff4b4b;">📌 {n_map.get(sel_tk, "")} ({sel_tk}) 투자 위원회 리포트</h3>
                                </div>
                                """,
                                unsafe_allow_html=True,
                            )
                            st.markdown(st.session_state[f"ai_{sel_tk}"])
